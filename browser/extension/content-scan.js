...
// Loaded after content.js (globals) and llm.js (LLM calls).
...
async function startScan() {
  ...
  try {
    ...
    attachedChatImages.push(await downscaleDataUrl(dataUrl));
    updateAttachBar();
    switchTab("chat");
...
async function processScan(task) {
  inFlightScan = true;
  try {
    await dispatchScan(task.dataUrl, task.rect, task.cacheKey);
  } finally {
    inFlightScan = false;
...
// Dispatcher for the capture-now pipeline: Ask mode → quiz solver, else translate.
async function dispatchScan(dataUrl, rect, cacheKey) {
  if (appMode === "ask") return runAskScan(dataUrl, rect, { allowAutoClick: true });
  return runTranslateScan(dataUrl, rect, cacheKey);
}

// One-shot vision translate: the LLM both reads and translates, returning
// fractional boxes we convert to rect-relative CSS px (also fixes the dpr
// mismatch the local server pipeline had).
async function runTranslateScan(dataUrl, rect, cacheKey) {
  try {
    setStatus("Dịch bằng Vision AI…");
    const sendUrl = await downscaleDataUrl(dataUrl);
    const blocks = await llmVisionTranslate(sendUrl, { glossary, characterNames });
    const items = blocks.map(b => {
      const bf = b.boxFrac || [0, 0, 0.4, 0.1];
      const memHit = b.src ? memoryGet(buildMemoryKey(b.src)) : undefined;
      return {
        box: [bf[0] * rect.width, bf[1] * rect.height, bf[2] * rect.width, bf[3] * rect.height],
        text: b.src || "",
        translation: memHit || b.vi,
      };
    });
    cacheSet(cacheKey, items);
    items.forEach(it => { if (it.text && it.translation) memorySet(buildMemoryKey(it.text), it.translation); });
    saveTranslationMemory();
    renderAnnotations(items, rect);
    setStatus(items.length ? `Đã dịch ${items.length} khối.` : "Không phát hiện chữ.", items.length === 0);
  } catch (error) {
    console.error(error);
    ...
    else setStatus(`Lỗi API: ${error.message}.`, true);
  }
}
...
async function _agentCallStep(dataUrl, viewportW, viewportH, stepHistory, task, mode = "act") {
  const sendUrl = await downscaleDataUrl(dataUrl);
  return llmAgentStep(sendUrl, {
    viewportW, viewportH,
    stepHistory: (stepHistory || []).slice(-6),
...
async function scanFullPage() {
  ...
  try {
    ...
    for (let i = 0; i < scrollTargets.length && scanMode; i++) {
      ...
      // Downscale each section before attaching to keep the chat payload small.
      attachedChatImages.push(await downscaleDataUrl(dataUrl));
      captured++;
    }
